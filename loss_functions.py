...
def cross_entropy_loss_lamda(y_true, y_pred):
...
def cross_entropy_balanced(y_true, y_pred):
    ...
    costNowieght = tf.nn.sigmoid_cross_entropy_with_logits(logits=y_pred, labels=y_true)
    #cost = tf.cond(beta > 0.6, lambda: cost, lambda: costNowieght )
        
    return tf.where(tf.greater(beta, 0.5), cost, costNowieght)
